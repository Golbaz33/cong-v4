...
# Version finale avec la méthode de filtrage et de recherche des congés maladie.
...
class DatabaseManager:
    ...
    def ajouter_conge(self, conge_model, cert_model=None):
        try:
            cursor = self.conn.cursor(); conge_id = self._ajouter_conge_no_commit(cursor, conge_model); self.conn.commit(); return conge_id
        except sqlite3.Error as e: self.conn.rollback(); raise e

    def modifier_conge(self, old_conge_id, new_conge_model, cert_model=None):
        try:
            cursor = self.conn.cursor(); self._supprimer_conge_no_commit(cursor, old_conge_id); new_conge_id = self._ajouter_conge_no_commit(cursor, new_conge_model); self.conn.commit(); return new_conge_id
        except sqlite3.Error as e: self.conn.rollback(); raise e

    def supprimer_conge(self, conge_id):
        try:
            cursor = self.conn.cursor(); self._supprimer_conge_no_commit(cursor, conge_id); self.conn.commit(); return True
        except sqlite3.Error as e: self.conn.rollback(); raise e
    
    def get_agents(self, term=None, limit=None, offset=None, exclude_id=None):
        q = "SELECT id, nom, prenom, ppr, grade, solde FROM agents"; p, c = [], []
    ...
    def get_agents_count(self, term=None):
        ...
        if term:
            t = f"%{term.lower()}%"; q += " WHERE LOWER(nom) LIKE ? OR LOWER(prenom) LIKE ? OR LOWER(ppr) LIKE ?"; p.extend([t, t, t])
        return self.execute_query(q, tuple(p), fetch="one")[0]

    def get_agent_by_id(self, agent_id):
        r = self.execute_query("SELECT id, nom, prenom, ppr, grade, solde FROM agents WHERE id=?", (agent_id,), fetch="one"); return Agent.from_db_row(r) if r else None
    ...
    def get_conge_by_id(self, conge_id):
        r = self.execute_query("SELECT id, agent_id, type_conge, justif, interim_id, date_debut, date_fin, jours_pris, statut FROM conges WHERE id=?", (conge_id,), fetch="one"); return Conge.from_db_row(r) if r else None
    ...
    def add_holiday(self, date_sql, name, h_type):
        try: self.execute_query("INSERT INTO jours_feries_personnalises (date, nom, type) VALUES (?, ?, ?)", (date_sql, name, h_type)); return True
        except sqlite3.IntegrityError: return False
    ...
    def get_maladies_sans_certificat(self):
        return self.get_sick_leaves_by_status(status='manquant')
    ...
    def get_agents_on_leave_today(self):
        """
        Récupère la liste des agents qui sont actuellement en congé.
        Un congé est considéré 'actuel' si la date d'aujourd'hui est entre sa date de début et de fin.
        """
        query = """
